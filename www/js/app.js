...
var mainApp = angular.module('starter', ['ionic', 'ionic.service.core',
  'ngSanitize'
]);
...
mainApp.controller('ListAllCategories', function($scope, $http, $stateParams,
  $rootScope) {
  ...
  $http.get('js/content.json').success(function(data) {
    ...
    angular.forEach(data.root.Main.ProductHolder.ProductPage, function(
      item, redniBroj) {

      $scope.listOfAllCategories.push({
        "catName": item.productCategory
      });
...
mainApp.controller('ListAllGenericName', function($scope, $http, $stateParams,
  $rootScope) {
  ...
  $http.get('js/content.json').success(function(data) {
    ...
    angular.forEach(data.root.Main.ProductHolder.ProductPage, function(
      ...
      $scope.listOfAllGenericNames.push({
        "catName": item.genericName
      });
...
mainApp.controller('ListByGenericName', function($scope, $http, $stateParams,
  $rootScope) {
  ...
  $http.get('js/content.json').success(function(data) {
    ...
    angular.forEach(data.root.Main.ProductHolder.ProductPage, function(
      ...
      if (decodeURIComponent($scope.selectedGeneric) == item.genericName) {
        ...
        $scope.listOfSelectedGenericItems.push({
          "genName": item.productName
        });
      }
...
mainApp.controller('SearchControl', function($http, $scope, Words, $rootScope,
  $stateParams) {
...
mainApp.controller('ProductCategoryContent', function($http, $scope, Words,
  ...
  $http.get('js/content.json').success(function(data) {
    ...
    angular.forEach(data.root.Main.ProductHolder.ProductPage, function(
      ...
      if (decodeURIComponent($scope.selectedCategory1) == item.productCategory) {
        ...
        $scope.listOfProductsForCat.push({
...
mainApp.controller('ProductDetailedContent', function($http, $scope, $rootScope,
  $stateParams) {
  ...
  $http.get('js/content.json').success(function(data) {
    ...
    angular.forEach(data.root.Main.ProductHolder.ProductPage, function(
      ...
      if (decodeURIComponent($scope.selectedProduct) == item.productName) {
        ...
        $scope.mainText = item.mainText;
        $scope.terapijskeIndikacije = item.terapijskeIndikacije;
        $scope.doziranjeINaicnPrimjene = item.doziranjeINaicnPrimjene;
        $scope.kontraindikacije = item["kontra-indikacije"];
        $scope.interakcijeSDrugimLijekovima = item.interakcijeSDrugimLijekovima;
        $scope.najeceNuspojave = item.najeceNuspojave;
  ...
  $scope.tab1act = "notactive";
  $scope.tab2act = "notactive";
  $scope.tab3act = "notactive";
  $scope.tab4act = "notactive";
  $scope.tab5act = "notactive";
  $scope.tab6act = "notactive";
  $scope.tab7act = "notactive";
  $scope.tab8act = "notactive";
...
mainApp.controller('Listbyletter', function($http, $scope, $rootScope,
  $stateParams) {
  ...
  $http.get('js/content.json').success(function(data) {
    ...
    for (v = 0; v < $scope.listOfWords.length; v++) {
      ...
      if (angular.lowercase($scope.listOfWords[v].itemname.substring(0,
          1)) === $scope.selectedLetter) {
...
mainApp.controller('ProductDetiledSidebar', function($http, $scope, $rootScope,
  $stateParams) {
  ...
  $http.get('js/content.json').success(function(data) {
    ...
    angular.forEach(data.root.Main.ProductHolder.ProductPage, function(
      ...
      if (decodeURIComponent($scope.productCategory2) == item.productCategory) {
        ...
        $scope.listOfProductsForSidemenu.push({
          "productname": item.productName
        });
...
mainApp.factory('Words', function($http, $rootScope) {
  ...
  function getData(wordname, callback) {
    ...
    $http.get('js/content.json').success(function(data, $rootScope) {
      ...
      //prolazi kroz svaku vrijednog jsona
      angular.forEach(data.root.Main.ProductHolder.ProductPage,
          function(item, redniBroj) {
            // dodjeljuje potrebne vrijednosti novom arrayu
            //  if($rootScope.langVal == "en"){

            listOfWords.push({
              "testname": item.productName
            });
